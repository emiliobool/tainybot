...
export const useSettingsStore = defineStore('settings', () => {
    ...
    const required = ['openai_api_key', 'twitch_access_token', 'channels', 'trigger_word', 'username']
    ...
    const changed = computed(() => {
        ...
        return JSON.stringify(settings.value) !== JSON.stringify(editingSettings.value)
    })
    ...
    const channels = computed(() => {
        return settings.value.channels?.split(',').map(c => c.trim()).filter(c => c.length > 0) || []
    })
