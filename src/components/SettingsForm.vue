...
let timeoutId = null;
const system_message_tokens = ref(0);
const system_message_value = ref("");

watchEffect(() => {
  system_message_value.value = system_message.value;
  if (!timeoutId) {
    system_message_tokens.value = settings.countTokens(
    ...
    <div class="py-6 px-4 sm:p-6 lg:pb-8">
      ...
      <div class="mt-6 flex flex-col lg:flex-row">
        <div class="flex gap-6">
          <div class="">
            ...
            <p class="mt-2 text-xs text-gray-500">
              ...
              <a
                href=""
                @click.prevent="
                  () => openLink('https://platform.openai.com/account/api-keys')
                "
                class="text-blue-500"
          ...
          <div class="">
            <label
              for="twitch_access_token"
              class="block text-sm font-medium leading-6 text-gray-900"
              >Twitch Bot Access Token*</label
            >
            <input
              type="password"
              v-model="twitch_access_token"
            ...
            <p class="mt-2 text-xs text-gray-500">
              ...
              <a
                ...
                href=""
                @click.prevent="
                  () => openLink('https://twitchtokengenerator.com/')
                "
                >https://twitchtokengenerator.com/</a
              >
            </p>
          </div>
        </div>
      ...
      <div class="sm:flex">
        ...
        <div class="flex-grow flex items-start">
          <div class="">
            <input
              ...
              type="checkbox"
              v-model="permissions_user"
              class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600"
            />
          </div>
          <div class="ml-3 text-sm leading-6">
            <label for="permissions_user" class="font-medium text-gray-900"
              >User</label
            >
          </div>
        </div>
        ...
        <div class="flex-grow flex items-start">
          <div class="">
            <input
              id="permissions_subscriber"
              name="permissions_subscriber"
              type="checkbox"
        ...
        <div class="flex-grow flex items-start">
          <div class="">
            <input
              id="permissions_vip"
              name="permissions_vip"
              type="checkbox"
              v-model="permissions_vip"
              class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600"
            />
          </div>
          <div class="ml-3 text-sm leading-6">
            <label for="permissions_vip" class="font-medium text-gray-900"
              >VIP</label
            >
          </div>
        </div>
        ...
        <div class="flex-grow flex items-start">
          <div class="">
            <input
              ...
              type="checkbox"
              v-model="permissions_mod"
              class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600"
            />
          </div>
          <div class="ml-3 text-sm leading-6">
            <label for="permissions_mod" class="font-medium text-gray-900"
              >Mod</label
            >
          </div>
        </div>
      ...
      <div>
        <div>
          <button
            class="text-sm rounded bg-slate-400 px-2 py-1 float-right text-slate-100"
            @click="resetContext"
          >
            Reset
          </button>
        ...
        <div class="flex justify-between">
          <p class="mt-2 text-xs text-gray-500">
            These are instructions on how the AI should behave. Nunjucks templating is used. The token count is an estimate before the template is processed.
          </p>
            <p class="mt-2 text-xs font-medium leading-6 text-gray-900 whitespace-nowrap">
              {{ system_message_tokens > 999 ? (system_message_tokens / 1000).toFixed(1) + 'k' : system_message_tokens }} Tokens
            </p>
        </div>
      </div>
      ...
      <div v-if="showAdvanced">
        ...
        <div class="sm:flex">
          ...
          <div class="flex-grow flex items-start">
            <div class="">
              <input
                id="send_to_twitch_chat"
                name="send_to_twitch_chat"
                type="checkbox"
            ...
            <div class="ml-3 text-sm leading-6">
              <label for="send_to_twitch_chat" class="font-medium text-gray-900"
                >Twitch Chat</label
              >
            </div>
          </div>
          ...
          <div class="flex-grow flex items-start">
            <div class="">
              <input
                id="send_to_text_file"
                name="send_to_text_file"
                type="checkbox"
            ...
            <div class="ml-3 text-sm leading-6">
              <label for="send_to_text_file" class="font-medium text-gray-900"
                >Text File</label
              >
            </div>
          </div>
        ...
        <div class="mt-6 grid grid-cols-12 gap-6">
          <div class="flex col-span-12 sm:col-span-6">
            <div class="flex-grow">
              <button
                class="float-right rounded bg-slate-400 py-1 px-2 text-sm text-slate-100"
                @click="resetModel"
              >
                Reset
              </button>
        ...
        <div>
          ...
          <div class="mt-2">
            <textarea
              id="preprocess_message"
              name="preprocess_message"
              rows="3"
              v-model="preprocess_message"
          ...
          <p class="mt-2 text-xs text-gray-500">
            Modify the message before sending it to the AI. Uses nunjucks
            templating.
          </p>
        </div>
        ...
        <div>
          ...
          <div class="mt-2">
            <textarea
              id="postprocess_message"
              name="postprocess_message"
              rows="3"
              v-model="postprocess_message"
              class="min-h-[100px] mt-1 block w-full rounded-md border-0 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-sky-500 sm:py-1.5 sm:text-sm sm:leading-6"
            />
          </div>
          <p class="mt-2 text-xs text-gray-500">
            Modify the message before sending it to twitch. Uses nunjucks
            templating.
          </p>
        </div>
